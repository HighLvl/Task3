...
public class RelationDao extends Dao<RelationEntity> {
    public RelationDao(Connection connection) throws SQLException {
        super(connection);
    }

    @Override
    protected PreparedStatement createInsertPreparedStatement() throws SQLException {
        return connection.prepareStatement("INSERT INTO relation(id, user_name, uid, version, changeset, timestamp, members, tags)\n" +
                "VALUES (?, ?, ?, ?, ?, ?, ?, hstore(?, ?))");
    }

    @Override
    protected String buildInsertQuery(RelationEntity entity) {
        return null;
    }

    @Override
    protected void prepareInsertStatement(RelationEntity entity) throws SQLException {
        ...
        insertPreparedStatement.setArray(7,
                connection.createArrayOf("member", entity.getMembers().toArray()));
